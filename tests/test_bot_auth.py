...
Two properties are under test, and they are the two that cannot be checked by
reading a handler:
...
from core.models import Entry, Member, PendingEntry
...
class FakeCallbackQuery:
    """Records every `answer()`, which is the whole point of half this file."""
...
async def test_a_rejected_callback_is_answered_exactly_once(
    ...
    # A callback query is answered with a toast, not a new message in the chat.
...
async def test_a_raising_handler_still_answers_exactly_once(
    ...
    rolled back before the `finally` ran.
    """
    world = await build_world(session)
